...
# ewang : 2/15/2016 add host name so that request can befrom the remote host
...
# ewang : 2/25/2016 change this msg if using host name
...
# ewang : 2/25/2016 remove this restriction
  #    if ($c) {
  ...
  #       }
      ...
      while ((defined $c) && (my $r = $c->get_request)) {
         if ($r->method eq 'POST' || $r->method eq 'GET') {
            $c->force_last_request;    # ewang: ensure not to read more requests from the connection
            my $uri = $r->uri;
...
#                  }
            ...
            my $resp="";
            if ($r->method eq 'POST') {
               my $q = new CGI($r->content());      # ewang : get post body
               $resp = process_request($q)
               }
            else {
               my $q = new CGI($r->uri->query());   # ewang : get query string
               $resp = process_request($q)
               }
...
# ewang : input either POST body string or GET query string
